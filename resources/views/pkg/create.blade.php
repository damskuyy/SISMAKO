...
@section('content')
    <div class="container mt-4">
        ...
        <form id="pkgForm" method="POST" action="{{ route('pkg.store') }}" enctype="multipart/form-data">
            @csrf
                ...
                <div class="card-body">
                    ...
                    <div class="pkg-step" data-step="1">
                        ...
                        <div class="row g-3 mb-3">
                            ...
                            <div class="col-md-3"><label>Nama</label><input type="text" id="nama" name="nama" class="form-control" value="{{ old('nama') }}" readonly required></div>
                            <div class="col-md-3"><label>NIP</label><input type="text" id="nip" name="nip" class="form-control" value="{{ old('nip') }}" inputmode="numeric" pattern="[0-9]*" readonly></div>
                            <input type="hidden" name="id_guru" id="id_guru" value="{{ old('id_guru') }}">
                            <input type="hidden" name="id_tendik" id="id_tendik" value="{{ old('id_tendik') }}">

                            <div class="col-md-4"><label>Mapel</label><input type="text" name="mapel" class="form-control" value="{{ old('mapel') }}"></div>
                            <div class="col-md-4"><label>Jabatan</label><input type="text" name="jabatan" class="form-control" value="{{ old('jabatan') }}"></div>
                            <div class="col-md-4"><label>Periode Penilaian</label>
                                <select name="periode_penilaian" class="form-control">
                                    ...
                                    @for($i=0;$i<6;$i++)
                                        ...
                                        <option value="{{ $p }}" {{ old('periode_penilaian') == $p ? 'selected' : '' }}>{{ $p }}</option>
                                    @endfor
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="pkg-step" data-step="2" style="display:none;">
                        ...
                        <div class="row g-3 mb-3">
                            ...
                            <div class="col-md-3"><label>Nama Penilai</label><input type="text" id="penilai_nama" name="penilai_nama" class="form-control" value="{{ old('penilai_nama') }}" readonly></div>
                            <div class="col-md-3"><label>NIP Penilai</label><input type="text" id="penilai_nip" name="penilai_nip" class="form-control" value="{{ old('penilai_nip') }}" inputmode="numeric" pattern="[0-9]*" readonly></div>
                            <div class="col-md-3"><label>Jabatan Penilai</label><input type="text" name="penilai_jabatan" class="form-control" value="{{ old('penilai_jabatan') }}"></div>
                            <input type="hidden" name="penilai_id_guru" id="penilai_id_guru" value="{{ old('penilai_id_guru') }}">
                            <input type="hidden" name="penilai_id_tendik" id="penilai_id_tendik" value="{{ old('penilai_id_tendik') }}">
                        </div>
                    </div>
                        ...
                        <div class="row g-3 mb-3">
                            <div class="col-md-3"><label>Kompetensi Pedagogik</label>
                                <select name="kompetensi_pedagogik" class="form-control">
                                    <option value="">-</option>
                                    @for($i=1;$i<=5;$i++)
                                        <option value="{{ $i }}" {{ old('kompetensi_pedagogik') == $i ? 'selected' : '' }}>{{ $i }}</option>
                                    @endfor
                                </select>
                            </div>
                            <div class="col-md-3"><label>Kompetensi Kepribadian</label>
                                <select name="kompetensi_kepribadian" class="form-control">
                                    <option value="">-</option>
                                    @for($i=1;$i<=5;$i++)
                                        <option value="{{ $i }}" {{ old('kompetensi_kepribadian') == $i ? 'selected' : '' }}>{{ $i }}</option>
                                    @endfor
                                </select>
                            </div>
                            <div class="col-md-3"><label>Kompetensi Profesional</label>
                                <select name="kompetensi_profesional" class="form-control">
                                    <option value="">-</option>
                                    @for($i=1;$i<=5;$i++)
                                        <option value="{{ $i }}" {{ old('kompetensi_profesional') == $i ? 'selected' : '' }}>{{ $i }}</option>
                                    @endfor
                                </select>
                            </div>
                            <div class="col-md-3"><label>Kompetensi Sosial</label>
                                <select name="kompetensi_sosial" class="form-control">
                                    <option value="">-</option>
                                    @for($i=1;$i<=5;$i++)
                                        <option value="{{ $i }}" {{ old('kompetensi_sosial') == $i ? 'selected' : '' }}>{{ $i }}</option>
                                    @endfor
                                </select>
                            </div>
                            <div class="col-md-12"><label>Keterangan</label><textarea name="kompetensi_keterangan" class="form-control">{{ old('kompetensi_keterangan') }}</textarea></div>
                        </div>
                    </div>
                        ...
                        <div class="row g-3 mb-3">
                            <div class="col-md-4"><label>Praktik Kinerja</label>
                                <select name="praktik_kinerja" class="form-control">
                                    ...
                                    <option value="Diatas Ekspektasi" {{ old('praktik_kinerja') == 'Diatas Ekspektasi' ? 'selected' : '' }}>Diatas Ekspektasi</option>
                                    <option value="Sesuai Ekspektasi" {{ old('praktik_kinerja') == 'Sesuai Ekspektasi' ? 'selected' : '' }}>Sesuai Ekspektasi</option>
                                    <option value="Dibawah Ekspektasi" {{ old('praktik_kinerja') == 'Dibawah Ekspektasi' ? 'selected' : '' }}>Dibawah Ekspektasi</option>
                                </select>
                            </div>
                            <div class="col-md-8"><label>Keterangan Praktik</label><input type="text" name="praktik_keterangan" class="form-control" value="{{ old('praktik_keterangan') }}"></div>

                            <div class="col-md-4"><label>Perilaku Kerja</label>
                                <select name="perilaku_kerja" class="form-control">
                                    <option value="">-- Pilih --</option>
                                    <option value="Diatas Ekspektasi" {{ old('perilaku_kerja') == 'Diatas Ekspektasi' ? 'selected' : '' }}>Diatas Ekspektasi</option>
                                    <option value="Sesuai Ekspektasi" {{ old('perilaku_kerja') == 'Sesuai Ekspektasi' ? 'selected' : '' }}>Sesuai Ekspektasi</option>
                                    <option value="Dibawah Ekspektasi" {{ old('perilaku_kerja') == 'Dibawah Ekspektasi' ? 'selected' : '' }}>Dibawah Ekspektasi</option>
                                </select>
                            </div>
                            <div class="col-md-8"><label>Keterangan Perilaku</label><input type="text" name="perilaku_keterangan" class="form-control" value="{{ old('perilaku_keterangan') }}"></div>
                            ...
                            <div class="col-md-4"><label>Predikat Kinerja</label>
                                <select name="predikat_kinerja" class="form-control">
                                    ...
                                    @foreach($preds as $p)
                                        <option value="{{ $p }}" {{ old('predikat_kinerja') == $p ? 'selected' : '' }}>{{ ucwords($p) }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-8"><label>Keterangan Predikat</label><input type="text" name="predikat_keterangan" class="form-control" value="{{ old('predikat_keterangan') }}"></div>
                        </div>
                    </div>
                    ...
                    <div class="pkg-step" data-step="5" style="display:none;">
                        ...
                        <div class="row g-3 mb-3">
                            <div class="col-md-12"><label>Kekuatan Guru</label><textarea name="kekuatan_guru" class="form-control">{{ old('kekuatan_guru') }}</textarea></div>
                            <div class="col-md-12"><label>Area Peningkatan</label><textarea name="area_peningkatan" class="form-control">{{ old('area_peningkatan') }}</textarea></div>
                            <div class="col-md-12"><label>Rekomendasi Tingkat Lanjut</label><textarea name="rekomendasi_tingkat_lanjut" class="form-control">{{ old('rekomendasi_tingkat_lanjut') }}</textarea></div>
                            <div class="col-md-6"><label>Foto Dokumentasi Kegiatan</label><input type="file" name="foto_dokumentasi_kegiatan" class="form-control"></div>
                        </div>
                    </div>
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    function validateStep(n){
        const el = document.querySelector('.pkg-step[data-step="'+n+'"]');
        if(!el) return true;
        // find first invalid control inside step using HTML5 validity
    ...
    if(personSelect){
        ...
        if(idGuruInput.value){
            ...
            // fallback: try to find option by name
    ...
    if(serverErrors && serverErrors.length){
        // find the first field name that exists in the form
        let targetStep = 1;
