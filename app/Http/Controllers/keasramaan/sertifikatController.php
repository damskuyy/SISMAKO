...
class sertifikatController extends Controller
{
    public function index(Request $request)
    {
        ...
        $sertifikat = sertifikat::with(['siswa:id,nama,nisn']);

        // Tambahkan filter tanggal jika ada input
        if ($startDate) {
            $sertifikat->where('tanggal', '>=', $startDate);
        }
        if ($endDate) {
            $sertifikat->where('tanggal', '<=', $endDate);
        }

        // Tambahkan filter nama jika ada input
        if ($searchName) {
            $sertifikat->whereHas('siswa', function ($query) use ($searchName) {
